...
impl ChatWidget {
            ...
            && name == "worktree"
        {
            ...
            match args.as_slice() {
                ...
                ["init"] => {
                    ...
                    let current_branch = codex_core::git_info::read_git_head_state(&head.head_path)
                ...
                ["init", name, branch] | ["init", name, branch, ..] => {
                    ...
                    tokio::spawn(async move {
                        ...
                        let command =
                            PlainHistoryCell::new(vec![Line::from(vec![invoked.magenta()])]);
                        tx.send(AppEvent::InsertHistoryCell(Box::new(
                            CompositeHistoryCell::new(vec![
                                Box::new(command),
                                Box::new(PlainHistoryCell::new(lines)),
    ...
    fn on_entered_review_mode(&mut self, review: ReviewRequest, _from_replay: bool) {
    ...
    fn open_kill_popup(&mut self) {
        ...
        items.push(SelectionItem {
            ...
            description: Some("Closes every running background terminal process.".to_string()),
        ...
        for process in &self.unified_exec_processes {
            let process_id = process.key.clone();
            let process_id_for_action = process_id.clone();
            let command_display = process.command_display.clone();
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            ...
            subtitle: Some("Select a unified-exec process id (shown by /ps).".to_string()),
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        for preset in presets.into_iter() {
            ...
            let _single_supported_effort = preset.supported_reasoning_efforts.len() == 1;
